/* THE RED CARPET DIRECTOR — Main Orchestrator */
(function () {
  'use strict';
  ...
  function initBottomNav() {
    ...
    if (!navItems.length || !scenes.length || !nav) return;
    ...
    navItems.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        if (window.LenisManager && LenisManager.lenis) { LenisManager.scrollTo(href); }
  ...
  function initMobileNav() {
    ...
    hamburger.addEventListener('click', function () { overlay.classList.contains('is-active') ? closeNav() : openNav(); });
  ...
  function initLightbox() {
    ...
    document.querySelectorAll('.archive-item, .filmstrip-item').forEach(function (item) {
    ...
    function closeLightbox() { lightbox.classList.remove('is-active'); document.body.style.overflow = ''; }
  ...
  function initAll() {
    ...
    safe(function () { initBottomNav(); }, 'BottomNav');
  ...
  var observer = new MutationObserver(function () {
    if (document.body.classList.contains('loaded')) { observer.disconnect(); setTimeout(initAll, 100); }
  });

  function boot() {
    observer.observe(document.body, { attributes: true, attributeFilter: ['class'] });
    setTimeout(function () { if (!inited) initAll(); }, 4000);
    setTimeout(function () { document.body.classList.remove('loading'); document.body.classList.add('loaded'); }, 5000);
  }
